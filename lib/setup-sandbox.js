...
const safeErrorHandler = {
	construct(Target, args) {
		return new Proxy(new Target(...args), {
			get: function get(innerTarget, prop, receiver) {
				if (prop === 'message' || prop === 'stack' || prop === 'prototype' ||
					prop === 'toString' || prop === 'code' || prop === 'name' || prop === 'length' || prop === 'arguments') {
					return Reflect.get(innerTarget, prop, receiver);
				} else if (prop === 'constructor') {
					const c = Reflect.get(innerTarget, prop, receiver);
	...
	get: function get(target, prop, receiver) {
		if (prop === 'captureStackTrace') {
			...
			return function innerGet(...opts) {
				const e1 = opts[0];
				const e2 = new e1.constructor(e1.message);
				for (const key in e1) {
					if (Object.prototype.hasOwnProperty.call(e1, key)) {
						e2[key] = e1[key];
					}
				}
				const ret = func(e2, opts[1]);
				opts[0].stack = e2.stack;
				return ret;
			};
		}
...
const safeSymbolHandler = {
	construct(Target, args) {
		return new Target(...args);
	},
	get: function get(target, prop, receiver) {
		if (prop === 'for') {
			const symbol = Reflect.get(target, prop, receiver);
			return function inner(...opts) {
				if (opts[0] === 'nodejs.util.inspect.custom' || opts[0] === 'Symbol.species' || opts[0] === 'nodejs.util.inspect.defaultOptions') {
					return {};
				}
				return symbol(...opts);
			};
...
const LocalError = new Proxy(Error, safeErrorHandler);
/* eslint-disable no-new-symbol */
const LocalSymbol = new Proxy(Symbol, safeSymbolHandler);
